...
diamonds <- diamonds
inputPanel(
  selectInput("variable1", label = "Choose Xaxis:", choices = c("carat","cut"), selected = "cut"),
  selectInput("variable2",  label = "Choose Yaxis:", choices = c("price","color"), selected = "color")
)
```
...
renderPlotly({
  x_axis <- input$variable1
  y_axis <- input$variable2
  clarity <- "clarity"
   a<-ggplot(diamonds, aes_string(x = x_axis, y = y_axis, color=clarity)) + geom_point() + facet_wrap(~clarity) + ggtitle("Plot of carat vs. price")
   ggplotly(a)
}) 
```
